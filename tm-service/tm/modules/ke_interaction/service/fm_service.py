...
from tm.modules.ke_interaction.interactions.fm_model import FMTSSplitURI,  DPSplitURI, \
     FMEvaluateResponseAsk, FMEvaluateQueryAsk
...
def evaluate_ask(query: List[FMEvaluateQueryAsk]) -> List[FMEvaluateResponseAsk]:
    response = []
    for q in query:
        ts_uri: FMTSSplitURI = FMTSSplitURI.parse(q.ts_uri)
        dp_uri = DPSplitURI.parse(q.dp)
        ...
        range_id = dao_manager.offer_dao.get_range(None, None).range_id
        # tou_uri = tou_model.TOUSplitURI(range_id=range_id, period_minutes=ts_uri.period_minutes, ts=ts)
        tou_price = FMEvaluateResponseAsk(ts_uri=q.ts_uri, dp=q.dp, ts=q.ts, dpr=q.dpr, value=q.value,
                                          cost_dp=dp_uri.uri_ref, cost_dpr=URIRef(dp_uri.uri + "/dpr"), cost=rdf_nil)
