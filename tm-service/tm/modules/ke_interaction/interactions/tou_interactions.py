...
def on_price_info_request(ki_id: str, bindings: List[TOUPriceInfoQuery]):
    ...
    prince_info_resp = tou_service.get_range_tou(tou_bindings, kb_id=tou_ki.get_kb_id())
...
def on_price_request(ki_id: str, bindings: List[TOUPriceQuery]):
    print("tou-price")
    # todo sprawdzic czemu tutaj sie wwala
    return tou_service.get_price(bindings, kb_id=tou_ki.get_kb_id())
